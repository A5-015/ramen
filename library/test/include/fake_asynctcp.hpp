...
class AsyncClient {
 public:
  ...
  void ack(int len) {
    if (_sent_cb) _sent_cb(NULL, this, len, 0);
  }
  ...
  bool operator==(const AsyncClient& other) { return mOther == other.mOther; }
...
inline bool AsyncClient::connect(IPAddress ip, uint16_t port) {
  ...
  void* arg = NULL;
  if (mServer->_connect_cb) mServer->_connect_cb(arg, this->mOther);
  if (this->_connect_cb) this->_connect_cb(arg, this);
  return true;
}
