...
def draw_static(type):
    ...
    hubs = {"x": [660, 1260, 1050, 1310], "y": [750, 1360, 920, 500]}
...
def draw_mesh():
    ...
    for node in mesh.node_list:
        ...
        if draw_node_range:
            circle = plt.Circle(
                (mesh.nodes[node].x, mesh.nodes[node].y),
...
def draw_star():
    ...
    star.find_neighbors(
        percent_network_coverage=star_percent_network_coverage, draw=False
    )
    star.place_hubs()
    star.create_links()
    ...
    for node in star.node_list:
        if node not in hubs:
            ...
            if draw_node_range:
                circle = plt.Circle(
                    (star.nodes[node].x, star.nodes[node].y),
    ...
    if draw_hubs:
        ...
        for hub in hubs:
            ...
            if draw_hub_range:
                circle = plt.Circle(
                    (star.nodes[hub].x, star.nodes[hub].y),
